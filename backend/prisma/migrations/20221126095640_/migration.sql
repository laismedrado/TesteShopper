/*
  Warnings:

  - You are about to drop the column `name` on the `orders` table. All the data in the column will be lost.
  - You are about to drop the column `usersId` on the `orders` table. All the data in the column will be lost.
  - A unique constraint covering the columns `[nickname]` on the table `users` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `nicknameId` to the `orders` table without a default value. This is not possible if the table is not empty.

*/
-- DropForeignKey
ALTER TABLE "orders" DROP CONSTRAINT "orders_usersId_fkey";

-- AlterTable
ALTER TABLE "orders" DROP COLUMN "name",
DROP COLUMN "usersId",
ADD COLUMN     "nicknameId" TEXT NOT NULL;

-- CreateIndex
CREATE UNIQUE INDEX "users_nickname_key" ON "users"("nickname");

-- AddForeignKey
ALTER TABLE "orders" ADD CONSTRAINT "orders_nicknameId_fkey" FOREIGN KEY ("nicknameId") REFERENCES "users"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
